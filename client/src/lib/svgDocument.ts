...
function readTagAt(
  content: string,
  index: number,
): { source: string; end: number; isClose: boolean; isSelfClosing: boolean; tagName: string } | null {
...
export function cursorOffsetForPath(
  content: string,
  path: PathSegment[],
): number | null {
  const node = findNodeByPath(parseIndexedDocument(content), path);
...
export function insertAttribute(
  ...
  if (existing !== undefined) {
    const valueCursor = attributeValueCursor(
      openTag,
      attrName,
      context.openTagStart,
    );
    if (valueCursor != null) {
...
export function insertChildElement(
  ...
  if (
    ...
    !schema.children.includes(normalizeTagName(childTag))
  ) {
    return null;
  }
...
export function deleteChildElement(
  content: string,
  path: PathSegment[],
): EditResult | null {
  const node = findNodeByPath(parseIndexedDocument(content), path);
  ...
  const next =
    content.slice(0, trimmed.start) + content.slice(trimmed.end);
